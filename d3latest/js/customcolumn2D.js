 ...
 */
var customColumn2D = function (chartId, chartdata, chartType) {
        ...
        data = options[0][selectedIndex].__data__;
                    if (selectedIndex != 0) {
                        if (chartdata.export.filename == undefined || chartdata.export.filename == '')
                            exportfile(chartId, chartdata, 'percentagediscount', '.' + data, false);
                        else
    ...
    .range(chartdata.chart.pallattecolor);
            ...
            var styleborder = "fill: none; stroke: white;  shape-rendering: crispEdges;font:12px sans-serif;color:white";
  ...
  .append("g")
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
     ...
     .text('(' + chartdata.chart.subhiddencaption.toUpperCase() + ')');
        ...
        .domain(labels)
        .rangeRoundBands([25, width-35], 0.1);
    ...
    var rectsnext = svg.selectAll('.customrectnext')
        ...
        .append('rect')
        .attr('x', function (d,i) {
            return xScale1(d.label.toString()) + xScale1.rangeBand() / 4;
        })
        .attr('y', function (d, i) {
            return yScale1(100);
        })
        .attr('height', function (d) {
            return yScale1(0) - yScale1(100);
             //yScale1(d.x0) - yScale1(d.x + d.x0);
        })
        .attr('class', function (d) {
            return 'customcolumnnext' + d.label.replace(/[^a-zA-Z0-9]/g, "");
        })
        ...
        .attr('fill',function(d){
            if(chartdata.sourcecolor != undefined){
                 if(chartdata.sourcecolor[d.label.toString().toUpperCase()] != undefined)
                    return chartdata.sourcecolor[d.label.toString().toUpperCase()];
                else
                   return color(0); 
            }
            else
     ...
     var rects = svg.selectAll('.customrect')
        ...
        .attr('height', function (d) {
            return yScale1(0) - yScale1(d.value);
        })
        .attr('class', function (d) {
            return 'customcolumn' + d.label.replace(/[^a-zA-Z0-9]/g, "");
        })
        .attr('data-visibility', true)
        .style('opacity', 1)
                .attr('width', function (d,i) {
      ...
      .call(xAxis().innerTickSize(-height + 25)
    .outerTickSize(0)
    .tickPadding(10));

      d3.selectAll(chartId+' .xtick .domain').attr('d','');
      d3.selectAll(chartId+' .xtick .tick text').attr('fill','white').style('stroke-width','0').style('font-size','1.2em').style('text-anchor','start').attr('transform',function(d,i){
        return 'translate('+(-xScale1.rangeBand() / 4)+',0)';
       });
       d3.selectAll(chartId+' .xtick .tick line').attr('transform',function(d,i){
        return 'translate('+(xScale1.rangeBand() / 2 + 10)+',0)';
       });
    ...
    d3.selectAll(chartId+' .yaxis .tick text').attr('fill','white').style('stroke-width','0').style('font-size','0.9em');
    d3.selectAll(chartId+' .yaxis .domain').style('stroke','white').style('stroke-width','0.9');
    ...
    .attr("transform", function(d) { 
        return "translate(" + (xScale1(d.label.toString())+xScale1.rangeBand()/4 -10 ) + "," + (yScale1(-9)) + ")"; 
    })
