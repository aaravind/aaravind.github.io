...
(function (window, $, undefined) {
	...
	if (typeof(JSON) == "undefined" ||  typeof(JSON.stringify) == "undefined") {
		$.cordys.loadScript(addOrganizationContextToURL("/cordys/html5/util/json2.js"));
	}
	if (typeof(btoa) == 'undefined') {
		$.cordys.loadScript(addOrganizationContextToURL("/cordys/html5/util/base64.js"));
	}
...
;(function (window, $, undefined) {
	...
	function createXMLElement(oXMLDoc, oParentEl, vValue, sName) {
		// Use createElementNS only in Firefox if we have xmlns attribute 
		if(vValue && vValue['@xmlns'] && (isFirefox || isOpera)) {
...
;(function (window, $, undefined) {
	...
	function makeRequest(request,opts){
		var dataStrings = [];
		if (request.method && request.namespace && !request.instanceid) {
		...
		{
		dataStrings.push("<SOAP:Envelope xmlns:SOAP='http://schemas.xmlsoap.org/soap/envelope/'><SOAP:Header><headerInput xmlns='http://schemas.cordys.com/bpm/instance/1.0'><target>");
			dataStrings.push(request.instanceid);
			dataStrings.push("</target></headerInput></SOAP:Header><SOAP:Body><");
			dataStrings.push(request.method);
			dataStrings.push(" xmlns='");
			dataStrings.push(request.namespace);
			dataStrings.push("' ");
	...
	function setCookiesToUrl(options) {
		...
		if (options.isMock !== true) {
			var ctCookie = $.cordys.getCookie("\\w*_ct"); // cookie name can be different, when property gateway.csrf.cookiename is set
			if (ctCookie) {
				options.url = $.cordys.addURLParameter(options.url, RegExp.$1, ctCookie);
			}
		}
	}
...
;(function (window, $, undefined) {
				...
				// Cordys SSO authentication
					...
					// check whether the ct is available in cookie or not
						...
						var samlArtInURL = $.cordys.getURLParameter(window.location, SAMLART_NAME),
							samlArtInCookie = $.cordys.getCookie(preloginInfo.SamlArtifactCookieName.text); //NOMBV
					...
					// nothing to do if ct is available in cookie
						...
						var samlArt = $.cordys.getCookie("\\w*_SAMLart");
	...
	function showLoginPage() {
		...
		if ($.mobile) {
			...
			window.showModalDialog($.cordys.addOrganizationContextToURL(loginURL));
			window.location.reload();
		}
	}
...
;(function (window, $, undefined) {
	...
	$.cordys.authentication.sso = {
		...
		isAuthenticated : function(){
			var ctCookie = $.cordys.getCookie("\\w*_ct");
	...
	function getGatewayURL() {
		var url = $.cordys.authentication.sso.defaults.loginGatewayURL,
			ctCookie = $.cordys.getCookie("\\w*_ct");
		
		// Add the ct name in the request URL
		if (ctCookie) {
			url = $.cordys.addURLParameter(url, RegExp.$1, ctCookie);
		}
		return url;
	}
